...
fix_permissions
log "Done"
##################### blocks until DB container is accepting TCP connections.##################
log "Waiting for TCP connection to ${DB_ADDRESS}"
while ! nc -w 1 ${DB_ADDRESSS} 3306 > /dev/null 2>&1
do
    echo -n .
    sleep 1
done
log "MySQL service is ready, Next......"
